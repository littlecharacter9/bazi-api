...
def get_prompt(bazi_str, gender, has_hour, module, year, liunian):
    ...
    style_control = """
【全局风格要求 - 必须严格遵守】
1. 语言风格：用朴实直白的日常口语，像朋友聊天一样。禁止使用任何诗句、古文、成语、歇后语、华丽词藻。
   比如不要说"金风玉露""鹏程万里""相得益彰""如日中天"，直接说"适合""有机会""互相配合""状态不错"。
2. 结构要求：先给出一句总体结论概括全貌，再分条详细说明。每条分析必须采用"结论 + 理由"的形式，
   先说判断结果，再简单说明为什么这么判断。理由要简短，一到两句话说清楚。
3. 格式要求：严格禁止使用任何 Markdown 标题符号，包括但不限于：#、##、###、****、==、---。
   用中文数字（一、二、三）和数字序号（1、2、3）来组织内容。
4. 人称要求：直接说"你"，不用"您""尔"等敬语或古语。
5. 内容末尾加上：AI生成内容仅供娱乐，理性看待。
"""
...
总体结论：（一句话概括{year}年整体运势）

一、整体运势：（{year}年对你来说是怎样的年份，说明理由）
二、事业方面：（工作上的情况，说明理由）
三、财运方面：（收入情况，说明理由）
四、感情方面：（感情上的情况，说明理由）
五、健康方面：（身体状况，说明理由）
六、后两年展望：（{year+1}年和{year+2}年的简要预测）
...
{style_control}
请根据八字推断以下内容，每条都要给出结论和理由：

一、环境方位
   （判断家里或家外的哪个方向有什么特征物品或环境，明确说是家里还是家外，给出理由）

二、六亲情况
   （判断与哪位亲人的关系如何，或该亲人的性格特征，给出理由）

三、过去经历
   （列举两个最有把握的年份发生过的事情，给出理由）

四、性格特征
   （列出2-3个明显的性格特点，给出理由）

输出完毕后请说：以上是我的初步推断，请你逐条判断是否准确。哪条不对请告诉我具体是哪条、哪里不对。"""
    }
...
def get_adjust_prompt(bazi_str, gender, section, original_content, user_feedback):
    ...
    style_control = """
【全局风格要求 - 必须严格遵守】
1. 语言风格：用朴实直白的日常口语，像朋友聊天一样。禁止使用任何诗句、古文、成语、华丽词藻。
2. 结构要求：先给结论，再说理由。每条分析采用"结论 + 理由"的形式。
3. 格式要求：禁止使用任何 Markdown 标题符号（#、##、###、****、==等）。
4. 人称要求：直接说"你"，不用"您""尔"。
"""
