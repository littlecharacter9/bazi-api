...
一、日主强弱
    根据是否得令、是否得地、是否得势三个方面，另外结合日柱空亡情况判断日元强弱；
    输出格式要求：得令、得地、得势三个方面分条阐述具体情况，如果有空亡影响则加此条 不受空亡影响则不阐述此条；最后给总结判断身强还是身弱
综合：日主XX，因为XX。
二、五行总论
① 五行平衡
...
五行总结：XXX
三、八字格局
① 正格：格局类型+原因+表现/影响(正格：比劫格、食伤格、财才格、官杀格、印枭格)
② 偏格：格局类型+原因+表现/影响(偏格：枭神夺食、伤官泄秀、伤官配印等等……)
