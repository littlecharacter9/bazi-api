...
def get_prompt(bazi_str, gender, has_hour, module, year, liunian):
    ...
    format_rule = """
【输出格式要求】
1. 严格按照给出的示例格式输出内容，填充内容只更改其中X的部分，其余格式保持不变
2. 除给出的格式外，填充内容要求语言朴实无华，不要用华丽辞藻(诗句古文)。
3. 不要使用"您""尔"等敬语，直接说"你"即可。
4. 结尾提示AI生成仅供参考(简洁提示就行)。
5. 禁止使用 Markdown 标题符号（#、##、###、****、==等）
"""
...
三. 颜色建议：多穿X色，少用X色。
    1.喜用颜色：根据喜用五行对应的颜色，X对应X、X对应X
    2.忌凶颜色：根据忌凶五行对应的颜色，X对应X、X对应X

四、数字建议：X、X、X是你的幸运数字。
    1.喜用数字：根据喜用五行对应的数字，X对应X、X对应X
    2.忌凶数字：根据忌凶五行对应的数字，X对应X、X对应X
    
五、方位建议：去X方发展对你有利
    1.喜用方位：根据喜用五行对应的数字，X对应X、X对应X
    2.忌凶方位：根据忌凶五行对应的数字，X对应X、X对应X
