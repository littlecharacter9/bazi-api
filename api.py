...
输出风格采用"描述+理由"方式（描述、理由分行显示），输出内容避免使用Markdown标题符号（#、##、###、****、==、---）
...
一、当前大运总览（{current_dy}，{current_age}）
1. 当前大运的优势
...
{style_control}
{da_yun_info}
请进行八字分析，输出：总体结论、核心判断、关键要点、建议、总结。每项附理由。
...
3. 如用户未明确说明不准的具体情况，如"不准""不清楚了"……，则重新生成此条目的其它推断(避免一直纠缠在同一推断不同解释)
